...
export function graphNodeRoleClass(role: TaskGraphNodeRole | undefined, selected: boolean, contextOnly: boolean | undefined) {
  ...
  if (role === "step_child" || role === "step_parent") return "border-violet-200 bg-violet-50 text-violet-950 dark:border-violet-900 dark:bg-violet-950/30 dark:text-violet-100"

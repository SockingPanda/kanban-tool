...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let task = snapshot
        ...
        .iter()
        .find(|t| t.id == "t_demo")
        .ok_or("demo task missing")?;
    tasks
        .update_task_title(UpdateTaskTitleRequest {
            board_id: "b_default".into(),
            task_id: task.id.clone(),
            title: "原生 gRPC 修改成功".into(),
            actor: "native-example".into(),
    ...
    while let Some(frame) = stream.message().await? {
        println!("{frame:?}");
        if matches!(frame.body, Some(board_frame::Body::Delta(_))) {
            break;
        }
    }
    Ok(())

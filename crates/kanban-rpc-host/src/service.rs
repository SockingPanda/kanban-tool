...
impl RpcApp {
    pub fn new(
        ...
        for hub in hubs {
            if by_id.insert(hub.board_id().to_owned(), hub).is_some() {
                return Err(LiveError::Invalid("重复 Hub".into()));
            }
        }
        if origins
            .iter()
            .any(|origin| origin == "*" || origin == "null")
        {
    ...
    fn hub(&self, board: &str) -> Result<Arc<Hub>, Status> {
        self.hubs
            .get(board)
            .cloned()
            .ok_or_else(|| error(LiveError::NotFound(board.into())))
    }
...
impl pb::board_service_server::BoardService for RpcApp {
    async fn get_board(
        &self,
        request: Request<pb::GetBoardRequest>,
    ) -> Result<Response<pb::GetBoardResponse>, Status> {
        self.origin(&request)?;
        let board = request.into_inner().board_id;
        let snapshot = self.hub(&board)?.snapshot().await.map_err(error)?;
        let response = pb::GetBoardResponse {
            board_id: board,
            cursor: Some(to_cursor(&snapshot.cursor)),
            tasks: snapshot.cards.values().map(to_card).collect(),
        };
        if response.encoded_len() > 4 * 1024 * 1024 {
            return Err(error(LiveError::Budget(
                "GetBoard 超过 4 MiB，请使用 WatchBoard 分块快照".into(),
            )));
        }
        Ok(Response::new(response))
    }
    type WatchBoardStream =
        Pin<Box<dyn Stream<Item = Result<pb::BoardFrame, Status>> + Send + 'static>>;
    async fn watch_board(
        &self,
        request: Request<pb::WatchBoardRequest>,
    ) -> Result<Response<Self::WatchBoardStream>, Status> {
        self.origin(&request)?;
        ...
        if input
            .resume
            .as_ref()
            .is_some_and(|r| r.epoch.len() > 128 || r.scope.len() > 256)
        {
        ...
        let mut cursor = input.resume.map(|c| Resume {
            epoch: c.epoch,
            scope: c.scope,
            revision: c.revision,
        });
                ...
                match hub.next(cursor.as_ref()).await.map_err(error)? {
                    ...
                    ReadNext::Idle => {
                        let changed = tokio::select! {
                            result = changes.changed() => Some(result),
...
impl pb::task_service_server::TaskService for RpcApp {
    async fn update_task_title(
        &self,
        request: Request<pb::UpdateTaskTitleRequest>,
    ) -> Result<Response<pb::UpdateTaskTitleResponse>, Status> {
        self.origin(&request)?;
        let input = request.into_inner();
        let expected = input
            .expected
            .ok_or_else(|| Status::invalid_argument("必须提供 expected version"))?;
        ...
        let result = tokio::time::timeout(
            ...
            self.commands.update_title(UpdateTitle {
        ...
        .await
        .map_err(|_| Status::deadline_exceeded("命令超时，结果可能已提交；请查询确认"))?
        .map_err(error)?;
        Ok(Response::new(pb::UpdateTaskTitleResponse {
            task: Some(to_card(&result)),
        }))
    }
}

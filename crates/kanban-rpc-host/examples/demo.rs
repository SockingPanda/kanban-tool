...
impl BoardSource for Demo {
    ...
    fn load_board<'a>(&'a self, board: &'a str) -> BoxFuture<'a, Result<Vec<Card>>> {
        Box::pin(async move {
            if board != "b_default" {
                return Err(LiveError::NotFound(board.into()));
            }
...
impl TaskCommands for Demo {
    fn update_title(&self, input: UpdateTitle) -> BoxFuture<'_, Result<Card>> {
        Box::pin(async move {
            if input.board_id != "b_default" {
                return Err(LiveError::NotFound(input.board_id));
            }
            let mut cards = self.cards.lock().await;
            let current = cards
                .get(&input.task_id)
                .ok_or_else(|| LiveError::NotFound(input.task_id.clone()))?;
            if current.lock_version != input.expected_version {
                return Err(LiveError::Conflict(input.task_id));
            }
            let mut next = current.clone();
            next.title = input.title;
            next.lock_version = next
                .lock_version
                .checked_add(1)
                .ok_or_else(|| LiveError::Budget("任务版本溢出".into()))?;
            next.validate()?;
...
async fn main() -> kanban_rpc_host::HostResult<()> {
    ...
    let demo = Arc::new(Demo {
        cards: Mutex::new(BTreeMap::from([(card.id.clone(), card)])),
        changed,
    });
    ...
    while hub.snapshot().await.is_err() {

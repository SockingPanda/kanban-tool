...
use kanban_protocol::{ApiBoardColumn, ListBoardColumnsPath, ListBoardColumnsResponse};
pub(crate) async fn list_board_columns(
    state: AppState,
    ListBoardColumnsPath { board }: ListBoardColumnsPath,
) -> Result<ListBoardColumnsResponse, ApiError> {
    let data = state
        .application()
        ...
        .collect();
    Ok(ListBoardColumnsResponse { data })
}

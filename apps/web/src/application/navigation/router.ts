...
export type BoardView = BoardRouteView

export type AppRoute =
  | { kind: "home"; pathname: string }
  | { kind: "board"; boardSlug: CanonicalBoardSlug; pathname: string; view?: BoardView; query?: string }
...
export function parseAppRoute(
  ...
  if (pathname.startsWith(boardPrefix)) {
    ...
    if (view === "signals" || view === "ontology") {
      const boardSlug = decodeBoardSlug(tail.slice(0, separator), pathname)
      if (typeof boardSlug !== "string") return boardSlug
      return { kind: "board", boardSlug, view: "list", query: "notice=page-removed", pathname: routePath({ kind: "board", boardSlug, view: "list" }, options) }
    }
    if (!(["board", "list", "map", "runs", "events"] as const).includes(view as BoardView)) {
      return { kind: "not-found", pathname }
    }
    const slug = separator > 0 ? tail.slice(0, separator) : tail
    const boardSlug = decodeBoardSlug(slug, pathname)
    if (typeof boardSlug === "string") {
      const route = { kind: "board" as const, boardSlug, pathname: routePath({ kind: "board", boardSlug, view: view as BoardView }, options) }
...
type RoutePathInput =
  | AppRoute
  | { kind: "home" }
  | { kind: "board"; boardSlug: CanonicalBoardSlug; view?: BoardView; query?: string }
  | { kind: "health"; boardSlug: CanonicalBoardSlug }
  | { kind: "maintenance"; boardSlug: CanonicalBoardSlug }
  | { kind: "settings" }
...
export function routePath(route: RoutePathInput, options: { basePath?: string } = {}): string {
  ...
  switch (route.kind) {
    ...
    case "board": {
      ...
      const pathname = `${basePath}boards/${encodeURIComponent(boardSlug)}/${view}`
...
export function useAppRouter(options: AppRouterOptions = {}) {
  ...
  useEffect(() => {
    const normalizeRemovedRoute = () => {
      const current = parseAppRoute(window.location.href, { basePath });
      if (current.kind === "board" && current.query === "notice=page-removed") {
        window.history.replaceState({}, "", routePath(current, { basePath }));
      }
    };
    normalizeRemovedRoute();
    const onPopState = () => {
      normalizeRemovedRoute();
      epochRef.current += 1
      setError(null)

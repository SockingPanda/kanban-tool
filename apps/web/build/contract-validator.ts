 ...
 */
export function createContractValidatorPlugin() {
  return {
...
function schemaPathForSlug(slug: string): string {
  ...
  const schemaPath = path.resolve(generatedSchemaDirectory, `${slug}.schema.json`)
  if (path.dirname(schemaPath) !== generatedSchemaDirectory) {

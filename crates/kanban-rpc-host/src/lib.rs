...
use kanban_rpc_proto::v1::{
    board_service_server::BoardServiceServer, task_service_server::TaskServiceServer,
};
...
pub async fn serve(
    ...
    let _lifetime = HostLifetime {
        app: app.clone(),
        connections: connections.clone(),
    };
    let incoming_connections = connections.clone();
    ...
    let server = Server::builder()
        ...
        .add_service(
            BoardServiceServer::new(app.clone())
                .max_decoding_message_size(64 * 1024)
                .max_encoding_message_size(4 * 1024 * 1024),
        )
        .add_service(
            kanban_rpc_proto::v1::workspace_service_server::WorkspaceServiceServer::new(
                app.clone(),
            )
        ...
        .add_service(
            TaskServiceServer::new(app)
                .max_decoding_message_size(64 * 1024)
                .max_encoding_message_size(64 * 1024),
        )
        .serve_with_incoming_shutdown(
            tokio_stream::wrappers::TcpListenerStream::new(listener)
                .map(move |stream| stream.map(|stream| incoming_connections.wrap(stream))),
            async move {
    ...
    let result = tokio::select! {
        ...
        _ = wait_stop(&mut forced) => {
            match tokio::time::timeout(Duration::from_secs(5), &mut server).await {
                ...
                Err(_) => Err(std::io::Error::new(std::io::ErrorKind::TimedOut, "RPC graceful shutdown 超时，已停止连接").into()),
            }
        }
    };
    // 超时只停止等待不足以回收 tonic 独立 spawn 的连接；显式唤醒其 IO。
    connections.abort();
    tokio::time::timeout(Duration::from_secs(1), connections.closed())
        .await
        .map_err(|_| std::io::Error::new(std::io::ErrorKind::TimedOut, "RPC 连接回收超时"))?;
    result
}
...
pub fn workspace_service(
    app: RpcApp,
) -> tonic_web::GrpcWebService<
    Deadline<kanban_rpc_proto::v1::workspace_service_server::WorkspaceServiceServer<RpcApp>>,
> {
    use tower::Layer;
    let service = kanban_rpc_proto::v1::workspace_service_server::WorkspaceServiceServer::new(app)
        .max_decoding_message_size(4096)
        .max_encoding_message_size(4096);
    tonic_web::GrpcWebLayer::new().layer(Deadline::new(service))
}

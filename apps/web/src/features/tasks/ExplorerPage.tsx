...
export function ExplorerPage(props:ExplorerPageProps) {
  ...
  return <div data-testid="explorer-page" onClickCapture={workspace.rememberTaskOpener} onKeyDownCapture={event=>{if(event.key==='Enter'||event.key===' ')workspace.rememberTaskOpener(event);}}>
    ...
    {view==='map'&&<TaskPage workspace={workspace}><TaskMapChunkBoundary><Suspense fallback={<p role="status">正在加载依赖图…</p>}><TaskMapView runtime={runtime} board={route.boardSlug} boardIdentity={workspace.mapIdentityRead.data} identityLoading={workspace.mapIdentityRead.loading} identityError={workspace.mapIdentityRead.error} onRetryIdentity={workspace.mapIdentityRead.retry} invalidationRevision={workspace.boardRevision} online={workspace.online!==false} taskId={workspace.taskId} onSelectTask={workspace.selectTask} urlState={workspace.mapUrlState} onUrlStateChange={workspace.updateMapUrlState} /></Suspense></TaskMapChunkBoundary></TaskPage>}
    {view==='runs'&&<TaskRunsPage workspace={workspace} />}
    {view==='events'&&<div className="standard-page"><EventsView runtime={runtime} boardSelector={route.boardSlug} taskId={workspace.taskId} kindFilter={workspace.kindFilter} online={workspace.online!==false} invalidationRevision={workspace.boardRevision} eventsRefreshRevision={workspace.eventsRefreshRevision} batch={workspace.eventsBatch} onKindFilterChange={workspace.updateEventKindFilter} onSelectTask={workspace.selectTask} /></div>}

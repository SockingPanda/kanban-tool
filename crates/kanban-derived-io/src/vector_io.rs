...
use kanban_vector::{
    ChunkBuilder, ChunkVectorStore, EmbeddingChunk, TaskChunkSource, VectorStoreBackend,
    VectorStoreStatus,
};
...
pub fn vector_chunks_for_entity_uris(
    conn: &Connection,
    board_id: &str,
    entity_uris: &[String],
    embedding_model: &str,
...
pub fn pending_vector_outbox_for_board(
    conn: &Connection,
    board_id: &str,
...
pub fn has_pending_vector_outbox_for_board(
    conn: &Connection,
    board_id: &str,
    last_event_id: Option<i64>,
) -> Result<bool> {
    crate::has_pending_outbox_for_target(conn, OutboxTarget::Lancedb, board_id, last_event_id)
}

pub fn rebuild_lancedb_chunks_with_store(
...
pub fn sync_lancedb_chunks_with_store(
    path: impl AsRef<std::path::Path>,
    board: &str,
    store: &impl ChunkVectorStore,
) -> Result<VectorStoreStatus> {
...
pub fn vector_store_status_with_conn(
    conn: &Connection,
    board_id: &str,

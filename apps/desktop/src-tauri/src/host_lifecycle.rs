...
pub(crate) fn probe_host_with_cancel(
    endpoint: SocketAddr,
    cancel: &AtomicBool,
    expected_web_build_id: Option<&str>,
) -> Result<HostCompatibility, ProbeError> {
    probe_host_with_timeouts_cancel_expected(
        endpoint,
...
fn probe_host_with_timeouts_cancel(
    ...
    probe_host_with_timeouts_cancel_expected(endpoint, timeouts, cancel, None)
}

fn probe_host_with_timeouts_cancel_expected(
    endpoint: SocketAddr,
    timeouts: ProbeTimeouts,
    cancel: Option<&AtomicBool>,
    expected_web_build_id: Option<&str>,
    ...
    probe_host_until_cancel_with_expected(
        endpoint,
        timeouts,
        deadline,
        cancel,
...
fn probe_host_until_cancel_with_expected(
    endpoint: SocketAddr,
    timeouts: ProbeTimeouts,
    deadline: Instant,
    cancel: Option<&AtomicBool>,
    ...
    if let Some(expected_web_build_id) = expected_web_build_id
        && compatibility.manifest.build_id != expected_web_build_id
    {
        return Err(ProbeError::incompatible(format!(
            "host webBuildId 不匹配本地 Web artifact: {} != {expected_web_build_id}",
            compatibility.manifest.build_id
        )));
    }
    if is_cancelled(cancel) {
        return Err(ProbeError::Cancelled);
...
pub struct HostLaunchConfig {
    ...
    /// 打包资源根目录；sidecar 必须是该目录的直接、canonical 子项。
    /// 测试和旧调用者未提供时退回 sidecar 的 parent，但生产 Desktop 总是显式传入。
    pub resource_dir: Option<PathBuf>,
    pub web_dir: PathBuf,
    /// 已验证的本地 Web artifact build id；生产 Desktop 必须设置，测试 seam 可省略。
    pub expected_web_build_id: Option<String>,
    pub db_path: PathBuf,
    pub actor: String,
    pub board: String,
...
impl HostLaunchConfig {
    pub fn new(
        ...
        let resource_dir = sidecar_path.parent().map(PathBuf::from);
        Self {
            endpoint: default_endpoint(),
            sidecar_path,
            resource_dir,
            web_dir: web_dir.into(),
            expected_web_build_id: None,
    ...
    pub fn with_expected_web_build_id(mut self, build_id: impl Into<String>) -> Self {
        self.expected_web_build_id = Some(build_id.into());
        self
    }
...
fn configure_owned_process_group(command: &mut Command) {
    // Linux 通过 `setsid(2)` 建立独立 session；这样 cleanup 向保存的 PGID 发信号时，
    // 不会误伤同一 session 中的外部进程。`pre_exec` 闭包只调用 async-signal-safe syscall，
    // 以满足 fork 后、exec 前的安全约束；不要同时设置 `process_group(0)`，否则 child
    // 先成为 group leader 后 `setsid` 会因 EPERM 失败。
    unsafe {
        command.pre_exec(|| {
            if libc::setsid() == -1 {
...
fn configure_owned_process_group(command: &mut Command) {
    // 非 Linux Unix 没有统一、可用的 session API；sidecar 仍成为独立 process group
    // leader，owned cleanup 只向保存的 PGID 发信号。
...
pub struct HostHandle {
    ...
    graceful_started_at: Option<Instant>,
...
impl HostHandle {
    ...
    pub(crate) fn shutdown_nonblocking(&mut self) -> Result<ShutdownResult, ShutdownError> {
            ...
            .is_some()
        {
            if finish_reaped_child(child) {
                self.child.take();
                return Ok(if self.force_requested {
                    ShutdownResult::Forced
                } else {
                    ShutdownResult::AlreadyExited
                });
            }
            return Err(ShutdownError::ReaperInFlight);
        }

        if self.force_requested {
            // SIGKILL/try_wait 也可能遇到短暂的 EINTR、子进程尚未 reap 或 descendant
            // group 仍在退出；每次 UI retry 都再次执行非阻塞 force cleanup，直到确认
            // process + group 均 gone，而不是把第一次 RetainForRetry 永久冻结成错误状态。
            let cleanup = force_stop(child);
            if cleanup.ownership() == CleanupOwnership::Release {
                child.finish_drain();
                child.release_process();
        ...
        let now = Instant::now();
        if let Some(started_at) = self.graceful_started_at {
            if now.duration_since(started_at) < GRACEFUL_SHUTDOWN_TIMEOUT {
                return Err(ShutdownError::GracefulInFlight);
            }
            let cleanup = force_stop(child);
        ...
        if let Err(error) = request_graceful_stop(child) {
            // SIGINT 发送失败时，sidecar 可能已经脱离旧 PGID；立刻进入同一 bounded
            // force 阶段，避免把 UI 的重试窗口耗尽在一个永远不会完成的 graceful 状态。
            self.force_requested = true;
            let graceful_error = ShutdownError::GracefulRequest(error.to_string());
            let cleanup = force_stop(child);
            if cleanup.ownership() == CleanupOwnership::Release {
                child.finish_drain();
                child.release_process();
        ...
        self.graceful_started_at = Some(now);
        Err(ShutdownError::GracefulInFlight)
    }
...
impl std::fmt::Display for ShutdownError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::GracefulInFlight => formatter.write_str("Desktop host graceful shutdown 进行中"),
            Self::WorkerInFlight => {
...
fn connect_or_spawn_inner(
    ...
    match probe_host_with_timeouts_cancel_expected(
        config.endpoint,
        config.probe_timeouts,
        cancel,
        config.expected_web_build_id.as_deref(),
    ) {
        Ok(_) => Ok(HostHandle {
            ownership: HostOwnership::External,
            child: None,
...
fn spawn_sidecar(config: &HostLaunchConfig) -> Result<OwnedChild, HostStartupError> {
    ...
    let trusted_path =
        trusted_sidecar_path(config).map_err(|message| HostStartupError::SidecarSpawn {
            path: config.sidecar_path.clone(),
            message: message.to_owned(),
        })?;
    let mut command = Command::new(&trusted_path);
    command
        .arg("--db")
        .arg(&config.db_path)
        .arg("--board")
...
impl SidecarTrustFacts {
    fn validate(self) -> Result<(), &'static str> {
        if self.symlink {
            return Err("sidecar 不得是符号链接");
        }
        if !self.regular {
            return Err("sidecar 必须是普通文件");
        }
        if !self.executable {
            return Err("sidecar 必须具有执行权限");
        }
        if self.writable_by_group_or_other {
            return Err("sidecar 不得允许 group/world 写入");
        }
        if let (Some(owner_uid), Some(current_uid)) = (self.owner_uid, self.current_uid)
            && owner_uid != 0
            && owner_uid != current_uid
        {
            return Err("sidecar owner 必须是 root 或当前用户");
        }
        Ok(())
    }
}
...
#[cfg(unix)]
fn sidecar_trust_facts(metadata: &Metadata) -> SidecarTrustFacts {
    let mode = metadata.mode();
    SidecarTrustFacts {
        regular: metadata.file_type().is_file(),
        symlink: metadata.file_type().is_symlink(),
        executable: mode & 0o111 != 0,
        writable_by_group_or_other: mode & 0o022 != 0,
        owner_uid: Some(metadata.uid()),
        // `geteuid` 避免在 setuid launcher 中把 real UID 与 effective UID 混淆。
        current_uid: Some(unsafe { libc::geteuid() }),
    }
}
...
fn trusted_sidecar_path(config: &HostLaunchConfig) -> Result<PathBuf, &'static str> {
    let metadata =
        fs::symlink_metadata(&config.sidecar_path).map_err(|_| "sidecar 文件不存在或无法读取")?;
    sidecar_trust_facts(&metadata).validate()?;

    let resource_dir = config
        .resource_dir
        .as_deref()
        .or_else(|| config.sidecar_path.parent())
        .ok_or("sidecar 缺少资源根目录")?;
    let resource_metadata =
        fs::symlink_metadata(resource_dir).map_err(|_| "sidecar 资源根目录不存在或无法读取")?;
    if resource_metadata.file_type().is_symlink() || !resource_metadata.file_type().is_dir() {
        return Err("sidecar 资源根目录必须是普通目录");
    }
    #[cfg(unix)]
    {
        let mode = resource_metadata.mode();
        if mode & 0o022 != 0 {
            return Err("sidecar 资源根目录不得允许 group/world 写入");
        }
        let owner_uid = resource_metadata.uid();
        let current_uid = unsafe { libc::geteuid() };
        if owner_uid != 0 && owner_uid != current_uid {
            return Err("sidecar 资源根目录 owner 必须是 root 或当前用户");
        }
    }
    let canonical_resource_dir = fs::canonicalize(resource_dir)
        .map_err(|_| "sidecar 资源根目录不存在或无法 canonicalize")?;
    let canonical_sidecar = fs::canonicalize(&config.sidecar_path)
        .map_err(|_| "sidecar 路径不存在或无法 canonicalize")?;
    if canonical_sidecar.parent() != Some(canonical_resource_dir.as_path()) {
        return Err("sidecar 必须位于资源根目录的直接子项");
    }
...
fn wait_for_sidecar_with_cancel(
    ...
    loop {
        ...
        let latest_error = match probe_host_until_cancel_with_expected(
            config.endpoint,
            config.probe_timeouts,
            probe_deadline,
            cancel,
...
mod tests {
    ...
    fn sidecar_trust_accepts_valid_direct_resource_child() {
        ...
        let config =
            HostLaunchConfig::new(&sidecar, root.path().join("web"), "db", "actor", "board");
        let trusted = trusted_sidecar_path(&config).expect("valid sidecar should pass trust gate");
        assert_eq!(
            trusted,
            std::fs::canonicalize(sidecar).expect("canonical sidecar")
        );
    }
    ...
    fn sidecar_trust_rejects_symlink_writable_and_escape_paths() {
        ...
        let symlink = root.path().join("kanban-link");
        std::os::unix::fs::symlink(&target, &symlink).expect("symlink fixture");
        let symlink_config = HostLaunchConfig::new(&symlink, "web", "db", "actor", "board");
        assert_eq!(
            trusted_sidecar_path(&symlink_config),
            Err("sidecar 不得是符号链接")
        );

        let writable = root.path().join("kanban-writable");
        std::fs::write(&writable, b"#!/bin/sh\nexit 0\n").expect("writable fixture");
        std::fs::set_permissions(&writable, std::fs::Permissions::from_mode(0o775))
            .expect("writable permissions");
        let writable_config = HostLaunchConfig::new(&writable, "web", "db", "actor", "board");
        assert_eq!(
            trusted_sidecar_path(&writable_config),
            Err("sidecar 不得允许 group/world 写入")
        );

        let outside = root
            .path()
            .join("..")
            .join(format!("kanban-trust-outside-{}", std::process::id()));
        ...
        let escape_config =
            HostLaunchConfig::new(&outside, root.path().join("web"), "db", "actor", "board")
                .with_resource_dir(root.path());
        assert_eq!(
            trusted_sidecar_path(&escape_config),
    ...
    #[cfg(unix)]
    #[test]
    fn sidecar_trust_rejects_foreign_owner_fact() {
        let current_uid = unsafe { libc::geteuid() };
        ...
        let error = SidecarTrustFacts {
            regular: true,
            symlink: false,
            executable: true,
            writable_by_group_or_other: false,
            owner_uid: Some(foreign_uid),
            current_uid: Some(current_uid),
        }
        .validate()
        .expect_err("foreign owner must fail trust gate");
        assert_eq!(error, "sidecar owner 必须是 root 或当前用户");
    }
    ...
    #[test]
    fn external_host_with_different_local_web_build_is_not_attachable_or_spawnable() {
        let (runtime, manifest) = compatible_values();
        let health = HealthResponse::new(HealthReport {
            ok: true,
            db: "turso".to_owned(),
        ...
        let error = probe_host_with_timeouts_cancel_expected(
            endpoint,
            ProbeTimeouts::default(),
            None,
            Some(expected),
        )
        .expect_err("foreign Web build must not attach");
        assert!(
            matches!(error, ProbeError::Incompatible(message) if message.contains("本地 Web artifact"))
        );
        ...
        let config = HostLaunchConfig::new("missing-sidecar", "web", "db", "actor", "board")
            .with_expected_web_build_id(expected);
        let error = connect_or_spawn(&HostLaunchConfig { endpoint, ..config })
            .expect_err("foreign Web build must not spawn a replacement on occupied port");
    ...
    fn startup_timeout_handoffs_owned_sidecar_to_reaper() {
        ...
        let sidecar = resource_root
            .path()
            .join(format!("kanban-desktop-startup-exit-sidecar-{id}"));
        ...
        let mut config = HostLaunchConfig::new(sidecar.clone(), "web", "db", "actor", "board")
            .with_resource_dir(resource_root.path());
        config.endpoint = endpoint;
    ...
    fn cancelled_startup_reaps_sidecar_descendants_before_returning_control() {
        ...
        let sidecar = resource_root
            .path()
            .join(format!("kanban-desktop-cancel-sidecar-{id}"));
    ...
    fn nonblocking_exit_cleanup_retries_until_descendant_group_is_gone() {
        ...
        let leader_pid = process.id() as libc::pid_t;
        assert_eq!(unsafe { libc::getpgid(leader_pid) }, leader_pid);
        assert_eq!(unsafe { libc::getsid(leader_pid) }, leader_pid);
        assert_ne!(unsafe { libc::getpgid(0) }, leader_pid);
        let marker_deadline = Instant::now() + Duration::from_secs(1);
    ...
    fn test_sleep_child_detached_from_owned_group() -> Child {
        ...
        command
            ...
            .stdout(Stdio::null())
            .stderr(Stdio::piped());
        // 不建立独立 session：test_owned_child 仍保存 child PID 作为“owned PGID”，
        // 因而 cleanup 的 group probe 看到 ESRCH，必须回退到 leader kill。

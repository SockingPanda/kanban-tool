...
Accepted；业务传输与实时协议由 [本地 gRPC 决策](0007_local_grpc.md) 更新。
...
- UI 采用 browser-first 架构，由 `kanban serve` 同源托管 `/app/`；浏览器和 Tauri 都加载同一份
  ...
  状态机。
- Web 通过同源 binary gRPC-Web 调用业务方法与完整查询流；HTTP 只提供启动健康探测与
  `/app/` 静态资源、runtime、manifest。SPA fallback 不得吞掉 RPC 或静态资源错误。
- `kanban-protocol` 是 Web wire 的唯一事实源。正式 Protobuf 生成类型、codec 和客户端；
  共享 DTO/schema 生成业务值、error、runtime 的 validator、fixture 与 contract hash。
  UI 数据在完整投影校验后发布；runtime 也由 protocol owner 生成。
...
- `GET /app/runtime.json` 提供空的同源 `apiBaseUrl`、`/app/` base path、actor、默认 board、server
  ...
  由 dev server 明确覆盖。
- `QueryService.WatchQueries` 在一条连接中复用当前活跃查询。身份包括 runtime、canonical board、
  过滤、排序、分页和投影版本；服务端共享相同查询的读取、快照和有界历史。
- 客户端完成 snapshot/delta 的基线、摘要、类型和作用域校验后原子发布数据与 cursor。断线保留
  最近已提交投影，并用该 cursor 恢复；历史缺失则重新获取快照。查询切换和卸载取消旧 generation，
  防止迟到数据覆盖当前页面。重试沿同一 RPC 协议进行。

### 视觉基线与平台范围

- 前端由 `app` 组合功能，`features` 持有领域组件，`application` 持有数据源接口、查询与异步
  操作，`domain` 持有展示模型与纯计算，`adapters/host` 接入生成契约与同源 gRPC-Web。
...
统一 Web artifact 和同源 host 减少了桌面/浏览器行为漂移，并把协议恢复和 runtime 版本检查集中
到可测试的边界；代价是 `kanban serve` 的静态资源装配、CSP、Host/Origin 校验和 build 一致性
成为所有入口的共同前置条件。完整查询流需要维护原子提交、恢复基线、查询共享与资源预算；
连接建立本身不能证明数据已经完成同步。

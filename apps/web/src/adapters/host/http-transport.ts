...
import { parseApiErrorResponse } from "../../lib/api/generated/contracts/api-error-response";

import { HttpTransportError, MAX_PATH_DECODE_LAYERS, type RequestURLOptions, type HttpTransportOptions, type JSONPayload, MAX_JSON_RESPONSE_BYTES, MAX_BINARY_RESPONSE_BYTES, REQUEST_HEADER_NAMES, type HttpTransport, type HttpRequestMethod, type HttpTransportResponse, type HttpTransportBytesResponse } from "../../application/data/http-transport";
...
export function malformedURL(message: string, cause: unknown): HttpTransportError {
  return new HttpTransportError("malformed_url", message, { cause })
}

export function sameOriginBase(runtime: WebRuntimeConfig, baseURI: string): URL {
  let documentURL: URL
  try {
...
export function hasDotSegmentOrBackslash(path: string): boolean {
...
export function hasMalformedPercent(value: string): boolean {
...
export function requestURL(base: URL, path: string, options: RequestURLOptions = {}): string {
...
export function validateCanonicalOpaqueSegment(segment: string, name: string, expectedPrefix: string): void {
...
export function attachmentRouteURL(base: URL, path: string): string {
...
export async function readJSON(response: Response): Promise<JSONPayload> {
...
export function binaryContentLength(response: Response): number {
  const value = response.headers.get("content-length")
  if (value === null) {
    throw new HttpTransportError(
      "invalid_bytes",
...
export async function cancelReader(reader: ReadableStreamDefaultReader<Uint8Array>): Promise<void> {
  try {
    await reader.cancel()
  } catch {
...
export async function readBytes(response: Response, declaredLength: number): Promise<Uint8Array> {
...
export function genericHTTPError(response: Response, cause?: unknown): HttpTransportError {
...
export async function throwByteHTTPError(response: Response): Promise<never> {
...
export function validateFinalOrigin(response: Response, requestOrigin: string): void {
...
export function validateFinalURL(response: Response, requestURLValue: string): void {
  let finalURL: URL
  let expectedURL: URL
  try {
    finalURL = new URL(response.url)

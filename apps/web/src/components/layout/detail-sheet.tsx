 ...
 */
export function DetailSheet({ open, title, description, onClose, children, footer, className = '', actions }: {
  open: boolean;
  title: string;
  description?: string;
  onClose: () => void;
  ...
  useEffect(() => {
    ...
    const escape = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !e.defaultPrevented && !document.querySelector('dialog[open], [data-choice-popup], [data-navigation-overlay]')) {
        e.preventDefault();
        latestClose.current();
      }
    };
  ...
  if (!open)
    return null;
  if (mobile)
    return <Dialog open side title={title} description={description} onClose={onClose} footer={footer} className={cn('detail-mobile', className)}>
  ...
  if (!host)
    return null;
  return createPortal(<section ref={panel} role="dialog" aria-modal="false" aria-labelledby={id} aria-describedby={description ? id + '-desc' : undefined} tabIndex={-1} className={cn('detail-sheet', wide && 'detail-wide', className)} data-testid="detail-sheet">

...
pub(crate) async fn update_step(
    ...
    let actor = actor(body.actor.as_deref(), &headers, &state);
    let parent = kanban_sqlite::get_task_by_id_global(state.db_path(), &parent_task_id)?;
    kanban_sqlite::update_step(
        state.db_path(),
        &parent.board_id,
        &actor,
        &parent_task_id,
        &step_id,
        kanban_sqlite::UpdateStepInput {
            title: body.title,
...
fn steps_dto(state: &AppState, task_id: &str) -> Result<TaskStepsDto, ApiError> {
    let task = kanban_sqlite::get_task_by_id_global(state.db_path(), task_id)?;
    let steps = kanban_sqlite::list_steps(state.db_path(), &task.board_id, task_id)?
        .into_iter()

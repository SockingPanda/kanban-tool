...
| `label.ontology.observation.recorded`、`label.ontology.action.created`、`label.ontology.signal.reviewed` | `E` | `S`: `label-ontology(board, *)`；`label.ontology.signal.reviewed` payload 带 `signal_id` 时同时失效 `label-ontology-signal(signal_id)`；observation event 的非空 `task_id` 额外失效该 task 的 `task-detail(task_id)`；`search-status(board)` | 这些是 `label_ontology_*` canonical ledger mutation 的 typed service events，与 generic `signal.*` 分离；摘要 payload 只用于定位，projection 仍从 ontology API 重新读取，不拼接伪实体 |

...
describe("label suggestions", () => {
  ...
  it("keeps long suggestion content constrained inside the task detail sheet", () => {
    ...
    expect(classListForPanelRoot(html)).toEqual(expect.arrayContaining(["min-w-0", "w-full", "max-w-full", "overflow-hidden"]))
    expect(classListForText(html, longSuggestionResult.selected_labels[0].label_name)).toEqual(expect.arrayContaining(["min-w-0", "max-w-full", "truncate"]))
    expect(classListForText(html, longSuggestionResult.selected_labels[0].evidence_atoms[0].text)).toEqual(expect.arrayContaining(["min-w-0", "max-w-full", "truncate"]))
...
function classListForPanelRoot(html: string) {
  ...
  while (searchEnd > 0) {
    ...
    if (classList.includes("space-y-2") && classList.includes("border")) return classList

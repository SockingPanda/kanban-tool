...
describe("signals and ontology API seam", () => {
  ...
  test("posts only supported lifecycle actions with a stable retry key and board scope", async () => {
    ...
    const request = vi.fn<NonNullable<SignalsOntologyReadTransport["request"]>>(async (options) => {
      void options
      return response({ data: action })
    })
    const transport: SignalsOntologyReadTransport = { get: vi.fn(), request }
    const api = createSignalsOntologyReadApi(runtime, {
      board: "default",
      identity: { selector: "default", canonicalBoardId: asCanonicalBoardId("b_1"), slug: "default", name: "Default" },
      transport,
    ...
    expect(request.mock.calls[0]?.[0]).toMatchObject({ method: "POST", path: "/api/v1/boards/b_1/label-ontology/actions" })
    const firstBody = (request.mock.calls[0]?.[0] as { body?: { idempotency_key?: string } }).body
    const secondBody = (request.mock.calls[1]?.[0] as { body?: { idempotency_key?: string } }).body
  ...
  test("rejects lifecycle responses outside the resolved board", async () => {
    const transport: SignalsOntologyReadTransport = {
      get: vi.fn(),
      request: vi.fn(async (): Promise<HttpTransportResponse> => response({
        data: {
          id: "loa_other",
          board_id: "b_other",
          parent_action_id: null,

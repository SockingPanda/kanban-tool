import { createSseParser } from "../../application/sync/sse-parser"
import type { SseTransport, SseTransportConnection, SseTransportRequest } from "../../application/sync/contracts"
...
function hasDotSegmentOrBackslash(path: string): boolean {
  ...
  for (const segment of pathOnly.split("/")) {
    if (segment === "../../application/sync/index" || segment === "..") return true
    try {
      let decoded = segment
      for (let pass = 0; pass < 64; pass += 1) {
        if (
          decoded === "../../application/sync/index"
          || decoded === ".."
          || decoded.includes("\\")
          || decoded.includes("/")
          || decoded.includes("\u0000")

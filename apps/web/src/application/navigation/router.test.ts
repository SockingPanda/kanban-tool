...
import { parseTaskMapUrlState } from "../../features/tasks/TaskMapView.logic"
...
describe("App route parser", () => {
  ...
  test.each(["signals", "ontology"])("redirects removed %s pages to the same board list", (view) => {
    const route = parseAppRoute('/app/boards/team-one/' + view + '?task=old&signal=secret');
    expect(route).toEqual({ kind: "board", boardSlug: "team-one", view: "list", pathname: "/app/boards/team-one/list", query: "notice=page-removed" });
    expect(routePath(route)).toBe("/app/boards/team-one/list?notice=page-removed");
  })

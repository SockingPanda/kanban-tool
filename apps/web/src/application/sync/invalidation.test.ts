...
describe("literal event invalidation plans", () => {
  ...
  test("removed feature events cannot create removed cache roots", () => {
    const plan = classifyEvent(event({ kind: "label.ontology.observation.recorded", taskId: null }));
    expect(plan.targets.some(target => /signal|ontology/.test(target.root))).toBe(false);
  })
  ...
  test("returns the conservative board plan for unknown events", () => {
    ...
    expect(plan.targets).toContainEqual({ root: "task-detail", boardId: "board-a", observedOnly: true })
  })

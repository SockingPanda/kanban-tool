...
impl TursoApplicationStore {
    ...
    pub fn with_run_log_root(store: TursoStore, run_log_root: Arc<PathBuf>) -> Self {
        Self {
            store,
    ...
    pub fn run_log_root(&self) -> Option<&Path> {
    ...
    pub async fn query_vector_chunks(
        ...
        model: &str,
        limit: usize,
    ) -> StoreResult<Vec<crate::VectorChunkHitRecord>> {
        self.store
            .query_vector_chunks(board_id, embedding, model, limit)
            .await
    }

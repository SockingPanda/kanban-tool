...
| `health.view` | `/app/boards/:board/health` / `none` | refresh、ok/db/version/db_path/db_fingerprint metrics、runtime config display | `GET /health`；无写 | loading/empty/no response/stale；`HealthErrorContent` 统一通过 `presentHealthError` 映射安全错误与下一步 | `features/health/HealthPage.test.tsx`、`features/health/health-error.test.ts`、`features/health/health-request.test.ts` | `P0-health`、`P1-health-error` | health 不是 event projection；SSE disconnect 不覆盖 health query | 纳入；同源 runtime/API identity 统一 |
| `maintenance.view` | `/app/boards/:board/maintenance` / `none` | stats/search status/global maintenance status；doctor、checkpoint、backup/export、portable import、vacuum、run/rebuild/cleanup；破坏性操作均经确认并显示 server 实际路径、checksum、bytes、fingerprint 等结果；legacy SQLite v30 只显示 honest unsupported，不渲染可执行按钮 | reads：`GET /api/v1/stats?board=`、`GET /api/v1/search/status?board=`、`GET /api/v1/maintenance/{status,doctor}`；writes：`POST /api/v1/maintenance/{checkpoint,backup,export,import,vacuum,run,rebuild,cleanup}` | 每 panel 独立 loading/empty/error；host/destructive/import 操作使用 Astryx `AlertDialog`，pending action 全局去重；status 在 mount、focus/visibility 和 ≤5s visible polling 时 refetch；成功 mutation refetch global status 并触发 `kanban:health-refresh` seam | `lib/api/maintenance-read-model.test.ts`、`features/maintenance/MaintenancePage.test.tsx`、`lib/api/http-transport.test.ts` | `tests/maintenance.spec.ts`（Chromium：status/unsupported、backup keyboard confirm/result evidence） | 当前 protocol 不存在 `maintenance.*`/`projection.*`；本地 mutation 只通过 typed HTTP seam，未知 event 仍由上层 conservative fallback；若未来接入 SSE 或 v30 import，必须先更新 protocol/schema/fixture/taxonomy | 纳入；不因 UI 重写引入 DB/schema migration |
| `settings.view` | `/app/settings` / `none` | system/light/dark theme、comfortable/compact density、zh/en locale；actor 本地偏好（仅校验/持久化，供 Stage05+07 mutation transport integration 接线）；只读 API origin/server version/Web build ID/health；诊断文本复制；当前 canonical Board session reconnect | `GET /health`；theme/locale/sidebar/density/actor 写 `kb:web:*` localStorage；reconnect 复用现有 Board session controller，不新建独立 SSE/poll；无 server write | health loading/error/stale/retry；无已解析 canonical board 时禁用 health/reconnect；actor 输入校验与运行时身份恢复；clipboard 成功/失败反馈；active session 已连接时报告无需 reconnect；reported fallback `not reported` | `lib/preferences.test.ts`、`features/settings/SettingsPage.test.tsx`、`features/board/board-session-registry.test.ts`、`tests/settings.spec.ts` | `P1-settings-locale-runtime` | settings 不订阅业务事件；health 由 query refetch；reconnect 仅作用于当前 active canonical session | 纳入；runtime/API identity 同源化，diagnostics 不暴露 database path，localStorage 使用新 `kb:web:*`；Stage05+07 integration blocker 必须在最终合并时接入 actor transport |

...
| `init` | `kanban init` | 保留命令名作 stop-gap，但不初始化 DB | exits `10 feature_not_available`, no storage touch | missing/retired |
...
|---|---|---|---|---|---|
| board `create`, `show`, `use`, `current`, `archive` | baseline `args.rs::BoardCommand` | owner：`kanban-service` board facts + `kanban-protocol` DTO；public entry：`kanban-server`/`kanban-client`、CLI config、Desktop board/settings operations | 先迁移 board mutation、`.kb/config` 解析和 archive guard，再开放嵌套 commands。 | clap、route、client、Desktop board settings 一致；当前嵌套命令保持 missing。 | missing |
...
| signal `record`, `list`, `show`, `review`, `confirm`, `reject`, `resolve`, `supersede` | baseline `args.rs`, `commands/signal.rs` | owner：`kanban-service` signal ledger；public entry：`kanban-server`/`kanban-client` signals HTTP、`kanban signal`、MCP signal tools、Desktop Signals | 先迁移 signal tables/backlinks，再复用同一 ledger operation 注册命令和 UI。 | signal lifecycle、backlink、JSON contract 和 Desktop review tests 完整。 | missing |
| hook `codex install`, `status`, `uninstall`, `handle failure`, `handle task-create` | baseline `args.rs`, `commands/hook.rs` | owner：host-admin/CLI hook adapter；public entry：`kanban hook codex ...` 与 managed hook files | 由 host-admin 明确 config root、原子写入/恢复和卸载边界，再注册 CLI hooks；禁止隐式写用户文件。 | install/status/uninstall/handle fixtures、权限和 recovery tests 完整。 | missing |
| config `show` | baseline `args.rs`, `commands/config.rs` | owner：CLI config resolver/host runtime；public entry：`kanban config show`, `.kb/config.toml` and Desktop runtime settings | 恢复配置优先级、locale/board/db source DTO，并使 serve/client 共享解析器。 | config show 只读、坏配置错误封装、无 DB touch 的 tests 完整。 | missing |
...
| hidden `dispatch`, `completions`, `__complete` | baseline `args.rs` | owner：host-admin dispatcher + CLI completion adapter；public entry：`kanban serve --dispatcher-profile`, `kanban completions`, hidden `__complete` | 将 dispatcher 绑定 serve host/claim path；completion 绑定 client contract，不重新打开 DB。 | profile/claim/shutdown、completion no-storage-touch tests 完整。 | partial/missing |

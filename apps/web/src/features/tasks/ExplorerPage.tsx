...
import { Component, lazy, Suspense, useCallback, useEffect, useLayoutEffect, useMemo, useRef, type MouseEvent, type KeyboardEvent, type ReactNode } from "react";
...
import { toBoardViewModel } from "../../application/tasks/board-model"
...
import { ExplorerReadError, serializeTaskListQuery, type TaskInspectorReadModel, type TaskListQueryState } from "../../application/data/explorer-read-model";
...
function taskListRow(task: import("../../application/data/explorer-read-model").ExplorerTaskListPage["tasks"][number]): TaskListRow {
  return {
    id: task.id,
    ref: task.ref,
    title: task.title,
...
function useExplorerPageState({ runtime, route, onNavigate, online, invalidationRevision = 0, boardRevision = invalidationRevision, inspectorRevision = invalidationRevision, runsRevision = invalidationRevision, eventsRefreshRevision = invalidationRevision, eventsBatch, syncStatus, taskMutations, onVisibleCanonicalReloadChange }: ExplorerPageProps) {
  const { loadTaskInspectorAttachments, loadTaskInspector, loadTaskInspectorEvents, loadTaskInspectorNeighborhood, loadTaskInspectorRuns, loadExplorerBoardIdentity, loadTaskListPage, createAttachmentDownloadClient, loadBoardReadModel } = useWorkspaceOperations();
  ...
  const boardRead = useAsyncRead(view === "board", route.boardSlug, (signal) => loadBoardReadModel(runtime, route.boardSlug, { signal }), boardRevision, online !== false)
...
export function ExplorerPage(props: Parameters<typeof useExplorerPageState>[0]) {
  const {
    rememberTaskOpener,
    route,
    copy,
    ...
    relationPanelVisible,
    attachmentsRead
  } = useExplorerPageState(props);
  return (
    <section className={styles.explorer} data-testid="explorer-page" onClickCapture={rememberTaskOpener} onKeyDownCapture={event => { if (event.key === "Enter" || event.key === " ") rememberTaskOpener(event); }}>
      {new URLSearchParams(route.query).get("notice") === "page-removed" ? <div role="status" className="removed-page-notice">该功能已移除，已返回当前项目任务列表。</div> : null}
      ...
      <div className={styles.content}>
        ...
        {showInspector ? (
          inspectorModel ? (
            <DetailSheet open title={inspectorModel.task.ref} onClose={closeInspector}><div className={styles.inspectorStack}>
              <TaskInspector
                ...
                dependencyContent={relationOwner ? (<TaskInspectorRelationsPanel section="dependencies"
              ...
              {relationOwner ? (
                <>
<TaskInspectorRelationsPanel section="details"
                    taskId={relationOwner.taskId}
                    comments={relationOwner.relations.comments}
                    dependencies={relationOwner.relations.dependencies}
                    steps={relationOwner.relations.steps}
          ...
          ) : <DetailSheet open title={copy.inspectorLoading} onClose={closeInspector}><InspectorBoundary loading={inspectorRead.loading} error={inspectorRead.error instanceof Error ? inspectorRead.error : null} onRetry={inspectorRead.retry} copy={copy} /></DetailSheet>
        ) : null}
      </div>
    </section>
  )

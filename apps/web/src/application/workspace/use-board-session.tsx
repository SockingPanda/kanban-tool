...
import type { AppNavigationTarget, AppRoute } from "../navigation/router"
import { parseCanonicalBoardSlug } from "../../domain/board-slug"
import { BoardReadError, type BoardReadModel } from "../data/board-read-model";
...
export function useBoardSession({ runtime, route, onNavigate, onSyncStatusChange, onTaskMutationsChange, onMutationCommitted, onCanonicalReload }: BoardLiveProps) {
  ...
  useEffect(() => {
    ...
    if (resource === undefined) {
      try {
        resource = makeResource(runtime, selector, source)

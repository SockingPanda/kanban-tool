...
impl pb::workspace_service_server::WorkspaceService for RpcApp {
    type WatchChangesStream = Pin<Box<dyn Stream<Item = Result<pb::WorkspaceChangeFrame, Status>> + Send + 'static>>;

    async fn watch_changes(&self, request: Request<pb::WatchChangesRequest>) -> Result<Response<Self::WatchChangesStream>, Status> {
        self.origin(&request)?;
        ...
        let source = self.refresh_source.clone().ok_or_else(|| Status::unimplemented("宿主未装配 RefreshSource"))?;
        let _permit = self.slots.clone().try_acquire_owned().map_err(|_| Status::resource_exhausted("最多 16 个活跃订阅"))?;
        let mut shutdown = self.shutdown.subscribe();
        if *shutdown.borrow() { return Err(Status::unavailable("宿主正在关闭")); }
            ...
            loop {
                tokio::select! {
                    ...
                    changed = changes.changed() => {
                        if changed.is_err() { Err::<(), _>(Status::unavailable("刷新源已关闭"))?; }
                        tokio::select! {
                            _ = wait_stop(&mut shutdown) => break,
                        ...
                        tokio::select! {
                            _ = wait_stop(&mut shutdown) => break,
                            result = tokio::time::timeout(Duration::from_secs(10), source.check_board(&board)) => {
                                result.map_err(|_| Status::deadline_exceeded("看板验证超时"))?.map_err(error)?;
                            }
                        }
...
fn invalidated(board: &str, epoch: &str, sequence: u64, reason: pb::RefreshReason) -> pb::WorkspaceChangeFrame {
    pb::WorkspaceChangeFrame { board_id: board.into(), epoch: epoch.into(), sequence,
        body: Some(pb::workspace_change_frame::Body::Invalidated(pb::QueriesInvalidated { reason: reason as i32 })) }
}

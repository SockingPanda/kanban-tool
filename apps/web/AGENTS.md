...
- `apps/web` 是 Browser 与 Tauri 共用的 product UI owner；两者加载 `kanban serve` 提供的同一
  `/app/` artifact。
- canonical mutation 经具名 binary gRPC-Web RPC 进入共享 service path；
...
- 基础组件由 `components/ui` 持有，领域组合由 `features` 持有，跨功能组合放在 `app`。
  公共出口显式列出，功能内部使用直接导入。
- `application` 持有数据源接口、异步操作和查询状态；`adapters/host` 接入正式生成契约、具名
  RPC 与完整 QueryService。`domain` 只保留展示模型与纯计算，功能组件不直接访问网络或存储。
...
- 同一数据源复用一个 QueryService registry；组件只消费完整已提交结果，显式刷新等待对应
  `Ready`。查询流协议与生命周期见 [查询订阅](docs/query-subscriptions.md)。
- `src/generated/rpc/` 只由根 Protobuf 生成工具写入。

...
describe("comments exact contracts",()=>{
 ...
 it("create sends exact transport while preserving locale and body actor semantics",async()=>{const fetch=vi.fn(async()=>response(create,201));vi.stubGlobal("fetch",fetch);await new KanbanApi(config,{locale:"zh-CN"}).createComment("t_fixture","decision");expect(fetch).toHaveBeenCalledOnce();const [url,init]=(fetch.mock.calls as unknown as [RequestInfo | URL, RequestInit][])[0]!;expect(url).toBe("http://127.0.0.1:8721/api/v1/tasks/t_fixture/comments");expect(init).toMatchObject({method:"POST",headers:{"Accept-Language":"zh-CN","Content-Type":"application/json"}});expect((init.headers as Record<string,string>)["X-KB-Actor"]).toBeUndefined();expect(JSON.parse(init.body as string)).toEqual({author:"desktop-test",body:"decision"})})

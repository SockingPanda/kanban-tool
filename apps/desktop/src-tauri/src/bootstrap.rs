...
impl DesktopHost {
    ...
    pub(crate) fn cancel_in_flight(&self) {
        ...
        if in_flight {
            ...
            let mut runtime = self.runtime.lock().expect("桌面 bootstrap 状态锁已失效");
            // 后台 worker 可能在两次 runtime 加锁之间完成；必须在执行 fence 的同一把
            // 锁下再次检查，确保成功启动仍保持 Ready。
...
pub(crate) fn start_background_attempt(
    ...
    // 注册闸门覆盖实际的线程创建和 worker 槽位安装；并发 close/cancel 只能
    // 在线性化点之前或之后生效，不能先观察到 open，再在 close 后留下已注册或已启动的 worker。
    ...
    let spawn_result = thread::Builder::new()
        ...
        .spawn(move || match kind {
            AttemptKind::ProbeExisting => {
                ...
                let result = match probe_host_with_cancel(
                    config.endpoint,
                    &cancel_for_thread,
...
#[cfg(target_os = "linux")]
fn trusted_xdg_open_path() -> Result<&'static str, BrowserOpenError> {
    let metadata = fs::symlink_metadata(XDG_OPEN_PATH).map_err(|error| {
        BrowserOpenError::BrowserLaunch(format!(
            "受信任的浏览器 launcher 不可用（{XDG_OPEN_PATH}：{error}）"
        ))
    })?;
    if metadata.file_type().is_symlink() || !metadata.file_type().is_file() {
        return Err(BrowserOpenError::BrowserLaunch(format!(
            "受信任的浏览器 launcher 不是普通文件：{XDG_OPEN_PATH}"
        )));
    }
    if metadata.mode() & 0o111 == 0 {
        return Err(BrowserOpenError::BrowserLaunch(format!(
            "受信任的浏览器 launcher 不可执行：{XDG_OPEN_PATH}"
        )));
    }
    Ok(XDG_OPEN_PATH)
}
...
pub(crate) fn open_in_browser(state: State<'_, DesktopHost>) -> Result<(), BrowserOpenError> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let launcher = trusted_xdg_open_path()?;
...
mod tests {
    ...
    use super::{
        ...
        BootstrapWorker, BrowserOpenError, CANCEL_RECHECK_PAUSED, CLOSE_GATE_PAUSED, DesktopHost,
        FIXED_APP_URL, PAUSE_AFTER_CLOSE_GATE, PAUSE_BEFORE_CANCEL_RECHECK,
        PAUSE_BEFORE_WORKER_INSTALL, ValidatedAppUrl, WORKER_INSTALL_PAUSED,
    };
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn browser_launcher_selection_is_absolute_and_trusted() {
        assert_eq!(super::XDG_OPEN_PATH, "/usr/bin/xdg-open");
        match super::trusted_xdg_open_path() {
            Ok(path) => assert_eq!(path, "/usr/bin/xdg-open"),
            Err(BrowserOpenError::BrowserLaunch(message)) => {
                assert!(message.contains("/usr/bin/xdg-open"));
            }
            Err(other) => panic!("unexpected launcher error: {other:?}"),
        }
    }

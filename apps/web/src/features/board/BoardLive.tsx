...
export interface BoardLiveProps {
  ...
  /** Keep the persistent board session alive while a feature route owns the visible content. */
  readonly renderBoardView?: boolean
}
...
export function BoardLive({ runtime, route, onNavigate, renderBoardView = true }: BoardLiveProps) {
  ...
  if (!renderBoardView) return null
  return (
    <BoardView
      state={visibleState}

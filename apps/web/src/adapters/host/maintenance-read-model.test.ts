...
import { type MaintenanceApiTransport } from "../../application/data/maintenance-api";
...
describe("maintenance API seam", () => {
  test("reads maintenance status, doctor, stats and search through generated contracts", async () => {
    ...
    const request = vi.fn<NonNullable<MaintenanceApiTransport["request"]>>(async () => response({ data: {} }))
    const api = createMaintenanceApi({ transport: { get, request } })
  ...
  test("rejects invalid maintenance status payloads at the generated contract seam", async () => {
    const api = createMaintenanceApi({
      transport: {
        ...
        request: vi.fn(async () => response({ data: {} })),
      },
    })
  ...
  test("uses the active POST routes and preserves server result evidence", async () => {
    ...
    await expect(api.maintenanceRun(" ")).resolves.toMatchObject({ action: "run" })

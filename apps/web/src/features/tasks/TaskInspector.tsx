import type { TaskInspectorViewModel, InspectorDependency, InspectorCopy } from './inspector-model';
export type { InspectorTaskStatus, InspectorPlanState, TaskInspectorViewModel, InspectorDependency, InspectorCopy } from './inspector-model';
...
import { createInspectorAsyncFence } from "../../application/tasks/inspector-section-state";
...
function useTaskInspectorState({ model, onSelectTask, locale = "zh", identity, refreshRevision = 0, refreshError, refreshOffline = false, online = true, onRetry, onLoadRuns, onLoadEvents, onLoadNeighborhood, mutationHandlers, mutationSnapshot, hideReadOnlyRelations = false, dependencyContent, claimToken = null }: TaskInspectorProps) {
  const { task } = model
  ...
  const [runsFence] = useState(createInspectorAsyncFence)
  const [eventsFence] = useState(createInspectorAsyncFence)
  const [neighborhoodFence] = useState(createInspectorAsyncFence)
...
export function TaskInspector(props: Parameters<typeof useTaskInspectorState>[0]) {
  const {
    copy,
    task,
    headingRef,

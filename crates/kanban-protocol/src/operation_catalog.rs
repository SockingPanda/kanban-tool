    ...
    ALL.get_or_init(|| {
        let mut declarations = Vec::with_capacity(
            crate::board_catalog::operation_declarations().len()
                ...
                + crate::knowledge_catalog::operation_declarations().len()
        ...
        declarations.extend_from_slice(crate::knowledge_catalog::operation_declarations());
...
mod tests {
    ...
    fn migrated_domain_source_is_exposed_without_legacy_duplication() {
        assert_eq!(operation_catalog().len(), 274);
        assert_eq!(
            operation_catalog()
                .iter()
            ...
            crate::board_catalog::operation_declarations()
                ...
                .chain(
                    crate::knowledge_catalog::operation_declarations()
                        .iter()
                        .map(|operation| operation.operation_id),
                )
                .chain(
                    crate::admin_catalog::operation_declarations()
                        .iter()
                        .map(|operation| operation.operation_id),
                )
                .chain(
                    crate::portable::operation_declarations()
                        .iter()
                        .map(|operation| operation.operation_id),
                )
                .chain(
                ...
                .chain(
                    crate::cli_labels_catalog::operation_declarations()
                        .iter()
                        .map(|operation| operation.operation_id),
                )
                .chain(

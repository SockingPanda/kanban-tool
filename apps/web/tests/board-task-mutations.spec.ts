...
const failure={error:{code:'service_unavailable',message:'暂时不可用'}};
...
test('失败后保留创建草稿，重试沿用 task id 与幂等键',async({page})=>{
  await installExplorerFixture(page);const bodies:Record<string,unknown>[]=[];
  await page.route('**/api/v1/boards/default/tasks',async route=>{
    if(route.request().method()!=='POST'){await route.fallback();return;}
    bodies.push(route.request().postDataJSON());
    if(bodies.length===1)await route.fulfill({status:503,json:failure});else await route.fallback();
  });
...
test('首个步骤失败后仅重试步骤，已创建任务不重复提交',async({page})=>{
  ...
  await page.route('**/api/v1/tasks/*/steps',async route=>{
    if(route.request().method()!=='POST'){await route.fallback();return;}
    steps++;if(steps===1)await route.fulfill({status:503,json:failure});else await route.fallback();
  });
  ...
  expect(steps).toBe(2);expect(fixture.writeRequests.filter(path=>path==='/api/v1/boards/default/tasks')).toHaveLength(1);
});

test('键盘和原生拖动都通过 claim，外部文本拖入不提交',async({page})=>{
  const fixture=await installExplorerFixture(page);let claims=0;
  await page.route('**/api/v1/tasks/t_ready/transitions/claim',async route=>{
    claims++;fixture.setReadyTaskStatus('running');
    await route.fulfill({json:{data:{claim_token:'claim-fixture',claim_expires_at:4102444800000,task:fixture.readyTask(),run:{id:'r_claim',task_id:'t_ready',status:'running',worker_profile:'manual',worker_pid:null,claim_owner:'local',started_at:1,finished_at:null,exit_code:null,summary:null,error:null,has_log:false,metadata:{}}}}});
  });
...
test('保存中的标题草稿等待服务，失败后可以编辑重试',async({page})=>{
  await installExplorerFixture(page);let release!:()=>void;
  const gate=new Promise<void>(resolve=>{release=resolve;});let attempts=0;
  await page.route('**/api/v1/tasks/t_ready',async route=>{
    if(route.request().method()!=='PATCH'){await route.fallback();return;}
    attempts++;if(attempts===1){await gate;await route.fulfill({status:503,json:failure});}else await route.fallback();
  });

...
async fn run(cli: &Cli) -> Result<(), CliFailure> {
    ...
    match &cli.command {
        ...
        Command::Attachment { command } => commands::attachment::run(&ctx, command).await,
        Command::Dependency { command } => commands::dependency::run(&ctx, command).await,
        Command::Entity { command } => commands::entities::run(&ctx, command).await,
        Command::Graph { command } => commands::graph::run(&ctx, command).await,
        Command::Events(args) => commands::event::run(&ctx, args).await,
        Command::Run { command } => commands::run::run(&ctx, command).await,
        Command::Signal { command } => commands::signal::run(&ctx, command).await,
        Command::Runs(args) => commands::run::list(&ctx, args).await,
        Command::Doctor => commands::maintenance::doctor(&ctx).await,
        Command::Stats(args) => commands::maintenance::stats(&ctx, args).await,
        Command::Backup(args) => commands::maintenance::backup(&ctx, args).await,
        Command::Export(args) => commands::maintenance::export(&ctx, args).await,
        Command::Import(args) => commands::maintenance::import(&ctx, args).await,
        Command::ImportV30(args) => commands::maintenance::import_v30(&ctx, args).await,
        Command::Checkpoint => commands::maintenance::checkpoint(&ctx).await,
        Command::Vacuum => commands::maintenance::vacuum(&ctx).await,
        Command::Maintenance(args) => commands::maintenance::maintenance(&ctx, args).await,
        Command::Task { command } => commands::task::run(&ctx, command).await,
        Command::Label { command } => commands::label::run(&ctx, command).await,
        Command::Search(args) => commands::search::run(&ctx, args).await,
        Command::Index { command } => commands::index::run(&ctx, command).await,
        Command::Vector { command } => commands::vector::run(&ctx, command).await,
        Command::Config { .. }
        | Command::Init { .. }
        | Command::Hook { .. }
        | Command::Completions { .. }

...
test('首页进入任务列表，四列看板保留全部活动状态',async({page})=>{
  ...
  expect(fixture.apiRequests.filter(path=>path === 'ListTasksByStatus')).toEqual([]);
...
test('无项目时展示空状态，不虚构任务或列',async({page})=>{
  ...
  expect(fixture.apiRequests.some(path=>path === 'ListBoardColumns')).toBe(false);
});
test('设置页只读取目录，不读取任务查询',async({page})=>{
  const fixture=await installBoardFixture(page);await page.goto('/app/settings');
  await expect(page.getByTestId('settings-page')).toBeVisible();
  expect(fixture.apiRequests.filter(path=>path === 'ListTasks')).toEqual([]);
});
test('QueryService 更新列表，离线保留快照并恢复连接',async({page,context})=>{
  ...
  await fixture.waitForQueryConnection(0);const initial=await fixture.getQueryConnectionCount();
  fixture.setReadyTaskTitle('QueryService 更新任务');await fixture.emitTaskUpdated();
  await expect(page.getByTestId('task-list')).toContainText('QueryService 更新任务');
  await context.setOffline(true);await expect(page.getByTestId('task-sync-notice')).toBeVisible();await expect(page.getByTestId('task-list')).toContainText('QueryService 更新任务');
  // 内存 QueryService 不经过浏览器网络层，显式断开以模拟真实连接断线。
  await fixture.closeQuery();
  await context.setOffline(false);await fixture.waitForQueryConnection(initial);
  await expect(page.getByTestId('task-list')).toContainText('QueryService 更新任务');
});

...
import { appendExplorerEventBatch, coalesceExplorerBoundary, explorerEventInvalidation, explorerEventInvalidationPlan } from "./session-events"
import type { ExplorerEvent } from "../data/explorer-read-model";
...
describe("Explorer invalidation boundary", () => {
  ...
  test("已删除功能的事件只更新动态，不恢复专属缓存", () => {
    ...
    expect(plan.targets).toEqual([
      { root: "events", boardId: asCanonicalBoardId("b_default") },
      { root: "search-status", boardId: asCanonicalBoardId("b_default") },
    ])

...
export class KanbanApi {
  ...
  async releaseTask(task: Task, claimToken: string, options: RequestOptions = {}): Promise<Task> {
    return this.transition(task, "release", { claim_token: claimToken }, options)
  }

  async transition(task: Task, action: "specify" | "promote" | "heartbeat" | "release" | "submit-review" | "reopen" | "unblock" | "archive", body?: Record<string, unknown>, options?: RequestOptions): Promise<Task>
  async transition(task: Task, action: "claim" | "complete" | "block", body?: Record<string, unknown>, options?: RequestOptions): Promise<Task | ClaimResponse>
  ...
  async transition(task: Task, action: "specify" | "promote" | "claim" | "heartbeat" | "release" | "complete" | "reopen" | "submit-review" | "block" | "unblock" | "archive", body: Record<string, unknown> = {}, options: RequestOptions = {}): Promise<Task | ClaimResponse> {
    ...
    if (action === "specify" || action === "promote" || action === "heartbeat" || action === "release" || action === "submit-review" || action === "reopen" || action === "unblock" || action === "archive") {

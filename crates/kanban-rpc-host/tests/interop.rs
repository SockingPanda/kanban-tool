//! 原生 HTTP/2 与 binary gRPC-Web 的真实网络互通。
...
async fn fixture() -> (
    ...
    let app = RpcApp::new(
        vec![hub.clone()],
        Arc::new(NoCommands),
        vec!["http://127.0.0.1:5173".parse().unwrap()],
    )
    .unwrap();
...
async fn native_and_grpc_web_use_same_protobuf_service() {
    ...
    let mut native = board_service_client::BoardServiceClient::connect(url.clone())
        .await
        .unwrap();
    let expected = native
        .get_board(GetBoardRequest {
    ...
    let response = reqwest::Client::new()
        .post(format!("{url}/kanban.framework.v1.BoardService/GetBoard"))
        .header("content-type", "application/grpc-web+proto")
        .header("x-grpc-web", "1")
        .header("origin", "http://127.0.0.1:5173")
        .body(web_body(GetBoardRequest {
            board_id: "b_default".into(),
        }))
        .send()
        .await
    ...
    let trailer = &bytes[5 + length..];
    assert_eq!(trailer[0], 0x80, "最后一个 gRPC-Web frame 必须是 trailer");
    let trailer_length = u32::from_be_bytes(trailer[1..5].try_into().unwrap()) as usize;
    assert_eq!(trailer.len(), 5 + trailer_length);
    let fields = std::str::from_utf8(&trailer[5..]).unwrap();
    assert!(
        fields.split("\r\n").any(|line| {
            line.split_once(':')
                .is_some_and(|(name, value)| name == "grpc-status" && value.trim() == "0")
        }),
        "缺少成功 trailer: {fields:?}"
    );
...
async fn native_stream_sends_snapshot_then_atomic_delta() {
    ...
    let mut native = board_service_client::BoardServiceClient::connect(url)
        .await
        .unwrap();
    let mut stream = native
        .watch_board(WatchBoardRequest {
            board_id: "b_default".into(),
            resume: None,
            protocol_version: 1,
        })
    ...
    loop {
        let frame = tokio::time::timeout(Duration::from_secs(2), stream.message())
            ...
            .unwrap();
        count += 1;
        if matches!(frame.body, Some(board_frame::Body::SnapshotCommit(_))) {
            break;
        }
    }
    assert_eq!(count, 3);
    hub.publish(vec![Card {
        title: "B".into(),
        lock_version: 1,
        ..card()
    }])
    ...
    let frame = tokio::time::timeout(Duration::from_secs(2), stream.message())
        ...
        .unwrap()
        .unwrap();
    let Some(board_frame::Body::Delta(delta)) = frame.body else {
...
async fn unknown_origin_is_rejected_before_command_dispatch() {
    let (url, _hub, stop, server) = fixture().await;
    let mut native = task_service_client::TaskServiceClient::connect(url)
        .await
        .unwrap();
    ...
    assert_eq!(
        native.update_task_title(request).await.unwrap_err().code(),

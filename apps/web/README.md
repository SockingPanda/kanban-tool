...
- 侧栏提供任务、模块、迭代、对象、项目动态、运行记录、设置和项目切换，收起后保留图标入口。
- 设置分为外观、操作身份和连接与诊断；数据库路径、指纹和构建信息放在默认收起的技术详情中，
  健康与数据维护入口保留服务诊断及确认流程。
- 模块、迭代与对象页面提供基础编辑、关系与附件；项目能力地图和 Git AI 追溯入口暂不展示。
...
对象页面保留未提交草稿的原版本；其他入口更新时显示冲突提示，保存仍由服务执行双版本 CAS。
对象属性可用 JSON 输入，迭代提供开始、关闭与取消操作。高级字段编辑器不在当前页面范围内。
附件组件共用于任务及其他对象，支持选择文件、拖放和粘贴；单文件最大 256 MiB，单个队列同时
传输两个文件。上传使用分块和稳定 ID，等待服务提交确认后再核对列表。文件可被多个对象引用，
解除引用只删除当前关系。下载先校验长度和 SHA-256；当前不提供在线预览或自动 blob 回收。

外观设置可以切换简体中文与英文，偏好在当前浏览器持久化。有效操作名称在失去焦点时保存，
非法草稿不会覆盖已保存身份。用户写入的标题、正文与文件名保持原文。
公共本地化运行时位于 `src/platform/localization`，使用打包的 i18next 资源、生成的键与参数类型
以及有界 Intl formatter 缓存；`application/i18n` 保留旧页面的兼容出口。语言支持按页面逐步接入，
当前严格扫描范围由 `build/i18n/config.json` 持有，不表示全站所有历史文案均已迁移。
...
其中 `web-i18n-check` 校验资源、参数、翻译记录和已迁移页面，`web-i18n-test` 验证工具与格式化行为。
`just web-i18n-proof` 构建并复制候选 Host 到独立目录，创建隔离数据库，验证语言、actor、对象与
附件的真实操作，并运行共享查询重连、原请求重试与任务附件的协议回归。证据记录 commit、diff、
候选 binary SHA-256、Web build ID 和运行时身份。
演示也使用隔离真实 Host；当前没有独立内存演示入口。

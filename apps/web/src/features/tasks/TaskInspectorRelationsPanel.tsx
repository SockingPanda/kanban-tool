...
} from "../../application/tasks/task-inspector-mutation-state"
...
export interface TaskInspectorRelationsPanelProps {
  readonly section?: "all" | "dependencies" | "details"
...
function CommentsPanel({
  ...
  useLayoutEffect(() => { draftRef.current = { kind, body } });
...
function DependenciesPanel({
  ...
  useLayoutEffect(() => { draftRef.current = input });
  const writePending = writePendingFor(snapshot, taskId)
...
function StepsPanel({
  ...
  useLayoutEffect(() => { draftRef.current = { title, body, required, linkedTaskRef, planReason } });
...
export function TaskInspectorRelationsPanel({
  ...
  return (
    <div className={styles.relations} data-testid="task-inspector-relations">
      {section !== "dependencies" && <CommentsPanel taskId={taskId} comments={comments} handlers={handlers} snapshot={snapshot} localeCopy={localeCopy} locale={locale} pageSize={safePageSize} />}
      {section !== "details" && <DependenciesPanel taskId={taskId} dependencies={dependencies} handlers={handlers} snapshot={snapshot} onSelectTask={onSelectTask} resolveSelector={resolveSelector} localeCopy={localeCopy} />}
      {section !== "dependencies" && <StepsPanel taskId={taskId} stepsInput={steps} handlers={handlers} snapshot={snapshot} onSelectTask={onSelectTask} resolveSelector={resolveSelector} localeCopy={localeCopy} />}
    </div>
  )
}

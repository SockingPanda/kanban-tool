...
where
    C: Clock,
{
    /// 执行一个已登记的 ontology operation，并为 mutation 保持 service 级串行语义。
    pub async fn label_ontology(
        ...
        if mutating {
            ...
            self.dispatch_label_ontology(operation, board, value).await
        } else {
            self.dispatch_label_ontology(operation, board, value).await
        }
    }

    async fn dispatch_label_ontology(
        &self,
        operation: &str,
        board: &str,
        value: Value,
        ...
        let result = match operation {
            ...
            "list_atoms" => json!(store.list_label_atoms(board).await.map_err(store_error)?),
            "explain_atom" => {

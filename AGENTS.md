...
- 并行 worktree / agent 开发时，kanban-tool 共享同一个项目级 Cargo target 根：`/media/kanban-user/Data/cargo-targets/kanban-tool`。
...
- 构建锁文件固定在 `/media/kanban-user/Data/cargo-targets/kanban-tool/.build.lock`。脚本会在等待时输出简洁中文提示，并在被包装命令成功、失败或中断后由本机文件锁自动释放。

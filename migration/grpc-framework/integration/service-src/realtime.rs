...
pub struct RealtimeTaskCard {
    pub id: String, pub title: String, pub status: TaskStatus, pub priority: i64,
    pub position: i64, pub seq: i64, pub lock_version: i64,
}
impl From<TaskRecord> for RealtimeTaskCard {
    fn from(task: TaskRecord) -> Self {
        Self { id: task.id, title: task.title, status: task.status, priority: task.priority,
            position: task.position, seq: task.seq, lock_version: task.lock_version }
    }
}
#[derive(Debug, Clone)]
pub struct RealtimeBoardSnapshot { pub board_id: String, pub tasks: Vec<RealtimeTaskCard> }
impl<C: Clock> KanbanService<C> {
    ...
    pub async fn load_realtime_board(&self, selector: &str) -> Result<RealtimeBoardSnapshot> {
        ...
        if board.archived_at.is_some() { return Err(KanbanError::NotFound(format!("看板 {}", board.id))); }
        let mut tasks = Vec::new();
        let mut offset = 0usize;
        let mut expected = None;
        let mut bytes = 0usize;
        loop {
            let page = self.list_tasks(&board.slug, TaskListOptions {
                limit: 1_000, offset, include_archived: false, ..TaskListOptions::default()
            }).await?;
            if page.total > 50_000 || expected.is_some_and(|total| total != page.total) {
                return Err(KanbanError::InvalidInput("看板快照过大或分页总量漂移".into()));
            }
            expected = Some(page.total);
            let size = page.tasks.len();
            if size == 0 && offset < page.total { return Err(KanbanError::Storage("快照分页没有前进".into())); }
            for task in page.tasks {
                let card = RealtimeTaskCard::from(task);
                bytes = bytes.checked_add(card.id.len() + card.title.len() + 128)
                    .ok_or_else(|| KanbanError::InvalidInput("快照大小溢出".into()))?;
                if bytes > 16 * 1024 * 1024 { return Err(KanbanError::InvalidInput("快照超过 16 MiB 数据预算".into())); }
                tasks.push(card);
            }
            offset += size;
            if offset >= page.total { break; }
        }
        if tasks.len() != expected.unwrap_or(0) { return Err(KanbanError::Storage("快照不完整".into())); }
        Ok(RealtimeBoardSnapshot { board_id: board.id, tasks })
    }
}

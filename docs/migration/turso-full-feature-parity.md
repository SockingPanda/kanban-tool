...
|---|---:|---:|---|
| Turso 表 | 35 | 38 | baseline 按 `migrations/001..030` replay 后的最终表名，忽略 `_new` 瞬态表；当前实现按 `crates/kanban-service/src/schema.rs` 的最终 schema 统计。38 张表还包含 schema identity、capability、import journal 等单主机治理表，因此不能用数量相等代替逐表语义验收。 |

...
| `settings.view` | `/app/settings` / `none` | system/light/dark theme、comfortable/compact density、zh/en locale；actor 本地偏好（仅校验/持久化，供 Stage05+07 mutation transport integration 接线）；只读 API origin/server version/Web build ID/health；诊断文本复制；当前 canonical Board session reconnect | `GET /health`；theme/locale/sidebar/density/actor 写 `kb:web:*` localStorage；reconnect 复用现有 Board session controller，不新建 SSE/poll；无 server write | health loading/error/stale/retry；no-board 时禁用 health/reconnect；actor 输入校验与保存反馈；clipboard 成功/失败反馈；reported fallback `not reported` | `lib/preferences.test.ts`、`features/settings/SettingsPage.test.tsx`、`features/board/board-session-registry.test.ts`、`tests/settings.spec.ts` | `P1-settings-locale-runtime` | settings 不订阅事件；health 由 query refetch；reconnect 仅作用于当前 active session | 纳入；runtime/API identity 同源化，diagnostics 不暴露 database path，localStorage 使用新 `kb:web:*`；Stage05+07 integration blocker 必须在最终合并时接入 actor transport |

import type { BoardRealtimeContext, BoardRealtimeController, BoardRealtimeSource, RealtimeState } from "./realtime-port.js"
import { sleep } from "./watch.js"
...
export function createChangeRealtime(key: string, open: OpenChanges, options: ChangeOptions = {}): BoardRealtimeSource {
  ...
  return {
    key,
    create(context) {
      if (!/^b_[^\s\u0000-\u001f\u007f]+$/.test(context.boardId) || !context.boardSelector.trim()) throw new Error("看板身份无效")

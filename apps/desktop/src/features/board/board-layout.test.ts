...
describe("board layout", () => {
  ...
  it("keeps board card rendering behind stable selection and memo boundaries", () => {
    ...
    expect(boardView).toContain("const BoardColumnBridge = memo(function BoardColumnBridge")
    expect(boardView).toContain("const BoardColumn = memo(function BoardColumn")
    expect(boardView).toContain("const selectedStatus = useMemo")
    expect(boardView).toContain("taskId={task.id}")
    expect(boardView).toContain("onSelectTask={onSelect}")
    expect(boardView).toContain("selectedId={column.status === selectedStatus ? selectedId : undefined}")
    expect(boardView).toContain("selected={task.id === selectedId}")
    ...
    expect(boardView).not.toContain("selectedIdForColumn(tasks, selectedId)")

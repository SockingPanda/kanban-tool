...
host 进程生命周期和可选 dispatcher。一个 loopback listener 同时接受原生 HTTP/2 gRPC 和
binary gRPC-Web；业务 adapter 复用共享 application。HTTP 只提供 `/health` 启动探针与
`/app/` 下的 Web 静态资源、runtime 和 manifest。`/` 与 `/app` 仅重定向到 `/app/`。
...
具名命令、查询、附件和管理操作由 `KanbanService` RPC 接收，持续完整查询由 `QueryService` 接收。
精确方法以 [正式 RPC catalog](../kanban-protocol/src/rpc/catalog.rs) 和生成 descriptor 为准；
...
查询 cursor 由 Host epoch、query scope 和 revision 组成，与审计 event ID 分开。断线后客户端只提交
已经完整验证的 cursor；历史已淘汰、query scope 改变或 Host 重启时接收完整快照重置。

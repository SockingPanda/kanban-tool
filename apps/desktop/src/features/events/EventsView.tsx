...
export function EventsView({ api }: { api: KanbanApi | null }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-card">
      <PageToolbar title={t("Board events")} className="py-2" meta={
      ...
      <ScrollArea className="flex-1 p-4">
        {events.length ? (
          ...
          <Empty>
            <EmptyDescription>{t("No events returned.")}</EmptyDescription>
          </Empty>
        )}
      </ScrollArea>
    </div>
    ...
    <Card className="grid grid-cols-[auto_1fr_auto] gap-3 px-3 py-2 text-sm">
      ...
      <div className="min-w-0">
        ...
        <div className="truncate text-xs text-muted-foreground">
          {event.task_id ? shortId(event.task_id) : t("board")} {event.run_id ? ` · ${shortId(event.run_id)}` : ""}
        </div>
      </div>
      <div className="text-right text-xs text-muted-foreground">
        <div>{formatRelativeTime(event.created_at)}</div>
        <div>{event.actor ?? t("system")}</div>
      </div>
    </Card>
  )
})

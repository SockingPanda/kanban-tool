...
pub(crate) struct MutationGate {
    ...
    #[cfg(test)]
    wait_samples: std::sync::Mutex<Option<Vec<GateWaitSample>>>,
}
...
impl MutationGate {
    pub(crate) fn new() -> Self {
        ...
        Self {
            ...
            revision,
            #[cfg(test)]
            wait_samples: std::sync::Mutex::new(None),
        }
    }
    ...
    pub(crate) async fn read(&self) -> MutexGuard<'_, ()> {
        ...
        #[cfg(test)]
        self.record_wait(true, started.elapsed());
        guard
    }
    ...
    pub(crate) async fn lock(&self) -> MutationGuard<'_> {
        ...
        let guard = self.mutex.lock().await;
        #[cfg(test)]
        self.record_wait(false, started.elapsed());
        MutationGuard {
            guard: Some(guard),
...
/// 仅在显式运行真实 service 负载测量时采集；产品构建没有计时或采样分支。
#[cfg(test)]
pub(crate) struct GateWaitSample {
    pub(crate) read: bool,
...
#[cfg(test)]
impl MutationGate {
    pub(crate) fn start_wait_measurement(&self) {
        *self.wait_samples.lock().unwrap() = Some(Vec::new());
    }

    pub(crate) fn take_wait_measurement(&self) -> Vec<GateWaitSample> {
        self.wait_samples.lock().unwrap().take().unwrap_or_default()
    }

    fn record_wait(&self, read: bool, elapsed: std::time::Duration) {
        if let Some(samples) = self.wait_samples.lock().unwrap().as_mut() {
            samples.push(GateWaitSample { read, elapsed });
        }
    }
}

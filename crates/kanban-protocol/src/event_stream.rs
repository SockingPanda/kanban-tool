...
/// CLI/MCP 事件消费兼容契约的非负安全整数 cursor 上限。
pub const MAX_SAFE_EVENT_CURSOR: i64 = 9_007_199_254_740_991;

/// 审计事件 envelope 的 canonical 顶层字段顺序。
...
mod tests {
    ...
    fn transport_heartbeat_is_not_a_business_envelope() {
        let heartbeat = EventHeartbeatData::default();
        assert_eq!(
            serde_json::to_value(heartbeat).unwrap(),
            serde_json::json!({})
        );
        assert!(
            serde_json::to_value(EventHeartbeatData::default())
                .unwrap()
                .get("id")
                .is_none()
        );
